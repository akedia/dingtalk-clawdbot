# 实际可用的老用户升级方案

## 问题说明

目前代码只在本地开发环境 (`E:\dingtalkclawd`)，还没有推送到 GitHub，所以之前文档中的 GitHub 链接都无法访问。

## ✅ 方案 1：通过内网服务器分享（推荐）

### 部署升级脚本到远端服务器

```bash
# 1. 上传升级脚本到远端
scp /e/dingtalkclawd/upgrade-from-v0.1.0.sh root@172.20.90.45:/tmp/

# 2. 设置可执行权限
ssh root@172.20.90.45 "chmod +x /tmp/upgrade-from-v0.1.0.sh"
```

### 老用户使用方式

**一键升级命令**（通过内网服务器）：
```bash
# 从你的服务器下载并执行
curl -fsSL http://172.20.90.45/upgrade-from-v0.1.0.sh | bash

# 或者通过 scp 下载
scp root@172.20.90.45:/tmp/upgrade-from-v0.1.0.sh /tmp/
bash /tmp/upgrade-from-v0.1.0.sh
```

---

## ✅ 方案 2：直接提供脚本文件

### 准备分享包

```bash
cd /e/dingtalkclawd

# 创建分享包
tar czf dingtalk-upgrade-kit.tar.gz \
  upgrade-from-v0.1.0.sh \
  分享给老用户.md \
  QUICK-UPGRADE.md \
  UPGRADE.md \
  COMPATIBILITY.md

echo "分享包已创建: dingtalk-upgrade-kit.tar.gz"
```

### 发给用户

**通过钉钉/微信/邮件发送** `dingtalk-upgrade-kit.tar.gz`

用户操作：
```bash
# 1. 解压
tar xzf dingtalk-upgrade-kit.tar.gz

# 2. 查看说明
cat 分享给老用户.md

# 3. 运行升级
bash upgrade-from-v0.1.0.sh
```

---

## ✅ 方案 3：直接 NPM 安装（最简单，无需脚本）

因为已经发布到 NPM，用户可以直接这样操作：

### 给用户的完整步骤（无需脚本）

```bash
# 步骤 1: 备份配置
cp ~/.clawdbot/clawdbot.json ~/.clawdbot/clawdbot.json.backup-$(date +%Y%m%d)

# 步骤 2: 保存 DingTalk 配置
python3 << 'EOF'
import json
with open(f"{__import__('os').path.expanduser('~')}/.clawdbot/clawdbot.json", 'r') as f:
    config = json.load(f)
dt = config.get('channels', {}).get('dingtalk', {})
if dt:
    with open('/tmp/dt-config.json', 'w') as f:
        json.dump(dt, f, indent=2)
    print("配置已保存")
EOF

# 步骤 3: 停止 gateway
pkill -f "clawdbot gateway" || true
sleep 2

# 步骤 4: 删除旧版本
rm -rf ~/.clawdbot/extensions/dingtalk

# 步骤 5: 临时清理配置引用
python3 << 'EOF'
import json
import os
cfg_file = f"{os.path.expanduser('~')}/.clawdbot/clawdbot.json"
with open(cfg_file, 'r') as f:
    config = json.load(f)
config.get('channels', {}).pop('dingtalk', None)
if 'plugins' in config and 'entries' in config['plugins']:
    config['plugins']['entries'].pop('dingtalk', None)
with open(cfg_file, 'w') as f:
    json.dump(config, f, indent=2)
print("配置已清理")
EOF

# 步骤 6: 安装新版本
clawdbot plugins install @yaoyuanchao/dingtalk

# 步骤 7: 恢复配置
python3 << 'EOF'
import json
import os
cfg_file = f"{os.path.expanduser('~')}/.clawdbot/clawdbot.json"
with open(cfg_file, 'r') as f:
    config = json.load(f)
try:
    with open('/tmp/dt-config.json', 'r') as f:
        dt = json.load(f)
    if 'channels' not in config:
        config['channels'] = {}
    config['channels']['dingtalk'] = dt
    if 'plugins' not in config:
        config['plugins'] = {}
    if 'entries' not in config['plugins']:
        config['plugins']['entries'] = {}
    config['plugins']['entries']['dingtalk'] = {'enabled': True}
    with open(cfg_file, 'w') as f:
        json.dump(config, f, indent=2)
    print("配置已恢复")
except:
    print("恢复失败，请手动从备份恢复")
EOF

# 步骤 8: 验证和启动
clawdbot doctor
clawdbot gateway
```

---

## ✅ 方案 4：创建 GitHub 仓库（长期方案）

如果要公开分享，需要先创建 GitHub 仓库：

### 创建仓库步骤

```bash
cd /e/dingtalkclawd

# 1. 如果还没有 GitHub 仓库，先创建
# 去 https://github.com/new 创建一个新仓库
# 名称: dingtalk-clawdbot

# 2. 添加远程仓库
git remote add origin https://github.com/yaoyuanchao/dingtalk-clawdbot.git

# 3. 推送代码
git push -u origin main
```

### 推送后用户就能用了

```bash
# 升级脚本可用
curl -fsSL https://raw.githubusercontent.com/yaoyuanchao/dingtalk-clawdbot/main/upgrade-from-v0.1.0.sh | bash

# 文档可访问
https://github.com/yaoyuanchao/dingtalk-clawdbot
```

---

## 📝 推荐的实际分享内容

### 给用户的消息（方案 3 - 最简单）

```
【DingTalk 插件升级通知 v1.2.0】

插件已发布到 NPM，支持官方安装！

🚀 升级方法（推荐）：

1. 备份配置：
cp ~/.clawdbot/clawdbot.json ~/.clawdbot/clawdbot.json.backup

2. 删除旧版本：
rm -rf ~/.clawdbot/extensions/dingtalk

3. 安装新版本：
clawdbot plugins install @yaoyuanchao/dingtalk

4. 恢复配置（从备份中复制 channels.dingtalk 配置回来）

5. 启动：
clawdbot gateway

✅ 配置完全兼容，staffId 白名单保持有效
✨ 新增：配置向导、健康检查、类型验证

详细步骤见附件或联系我
```

### 附上文件

从 `E:\dingtalkclawd\` 打包这些文件发给用户：
- `分享给老用户.md`
- `upgrade-from-v0.1.0.sh`
- `QUICK-UPGRADE.md`

---

## 🎯 当前最实用的方案对比

| 方案 | 优点 | 缺点 | 推荐度 |
|------|------|------|--------|
| **方案 3: 直接步骤** | 不依赖外部资源 | 步骤稍多（8步） | ⭐⭐⭐⭐⭐ |
| 方案 2: 分享包 | 包含完整文档 | 需要传文件 | ⭐⭐⭐⭐ |
| 方案 1: 内网服务器 | 一键执行 | 需要配置HTTP服务 | ⭐⭐⭐ |
| 方案 4: GitHub | 最标准 | 需要先创建仓库 | ⭐⭐ |

---

## ⚡ 立即可用的升级命令（复制给用户）

### 最简版本（3 步）

```bash
# 1. 备份
cp ~/.clawdbot/clawdbot.json ~/.clawdbot/clawdbot.json.backup-$(date +%Y%m%d)

# 2. 删除旧版本并安装新版本
rm -rf ~/.clawdbot/extensions/dingtalk && \
clawdbot plugins install @yaoyuanchao/dingtalk

# 3. 手动恢复配置
# 编辑 ~/.clawdbot/clawdbot.json
# 从 ~/.clawdbot/clawdbot.json.backup-YYYYMMDD 复制 channels.dingtalk 配置回来
# 添加 plugins.entries.dingtalk: {"enabled": true}
```

然后启动：
```bash
clawdbot gateway
```

---

## 🔧 我来帮你做什么？

选择一个方案，我可以帮你：

1. **创建分享包** - 打包所有文件供你发给用户
2. **部署到内网服务器** - 上传脚本到 172.20.90.45
3. **创建 GitHub 仓库** - 推送代码到 GitHub
4. **生成简化版说明** - 只包含必要步骤的简单文档

你想用哪个方案？
